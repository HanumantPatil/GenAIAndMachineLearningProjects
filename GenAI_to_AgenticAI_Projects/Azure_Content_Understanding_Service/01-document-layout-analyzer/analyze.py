...
def create_client() -> tuple[ContentUnderstandingClient, DefaultAzureCredential | None]:
    """Create a client using an API key or Microsoft Entra credentials."""
...
def to_dictionary(value: Any) -> dict[str, Any]:
    """Convert an Azure SDK model to a JSON-compatible dictionary."""
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract text, tables, sections, and layout from a document."
    )
    parser.add_argument("input_source", help="Local path or public URL of a PDF or image.")
    parser.add_argument(
        "--output",
        type=Path,
        help="Optional path for the full JSON result, for example output/result.json.",
    )
...
def main() -> int:
    ...
    try:
        ...
        print("Submitting the document for analysis...")
        ...
        print("\n--- Extracted Markdown ---\n")
        print(content.markdown or "No markdown content was returned.")
        ...
        if args.output:
            ...
            args.output.write_text(
                json.dumps(to_dictionary(result), indent=2, default=str),
                encoding="utf-8",
            )
