"""Streamlit UI for the document layout analyzer."""
...
st.caption("Extract text, tables, sections, and document structure with Azure Content Understanding.")
...
if st.button("Analyze document", type="primary", disabled=source_input is None, use_container_width=True):
    ...
    try:
        ...
        if not result.contents:
            ...
            markdown_tab, json_tab = st.tabs(["Extracted content", "Full JSON"])
            with markdown_tab:
                st.markdown(result.contents[0].markdown or "No markdown content was returned.")
            with json_tab:
                st.json(result_dictionary)

            result_json = json.dumps(result_dictionary, indent=2, default=str)
            st.download_button(
                "Download JSON result",
                data=result_json,
                file_name="document-layout-result.json",
