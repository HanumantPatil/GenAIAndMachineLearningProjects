"""Streamlit UI for the US tax analyzer."""
...
st.set_page_config(page_title="US Tax Form Analyzer", layout="wide")
...
def run_analysis(input_item: AnalysisInput) -> tuple[Any, dict[str, Any]]:
    """Submit one tax form and return the SDK and JSON-compatible results."""
    credential = None
    try:
...
def render_fields(extracted_fields: dict[str, Any]) -> None:
    """Render extracted tax values and confidence scores."""
    if not extracted_fields:
        st.info("No tax fields were returned.")
        return
...
st.title("US Tax Form Analyzer")
st.caption("Classify US tax documents and inspect extracted fields with Azure Content Understanding.")
...
source_type = st.segmented_control("Tax document source", ["Upload", "Public URL"], default="Upload")
if source_type == "Upload":
    uploaded_source = st.file_uploader("Choose a tax document", type=["pdf", "png", "jpg", "jpeg", "tif", "tiff"])
    source_input = create_uploaded_input(uploaded_source) if uploaded_source else None
else:
    source_url = st.text_input("Public tax document URL", placeholder="https://example.com/w2.pdf")
    source_input = AnalysisInput(url=source_url.strip()) if source_url.strip() else None

if st.button("Analyze tax form", type="primary", disabled=source_input is None, use_container_width=True):
    if source_input is None:
        st.stop()
    try:
        with st.spinner("Classifying and extracting the tax form..."):
        ...
        if not result.contents:
            ...
            st.metric("Detected tax form", content.category or "Unknown")
